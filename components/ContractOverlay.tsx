...
export const ContractOverlay: React.FC<ContractOverlayProps> = ({ user, template, onSigned }) => {
    ...
    const handleScroll = () => {
        ...
        // Se o usuário chegou a 95% do fim, libera o botão
    ...
    return (
        <div className="fixed inset-0 z-[200] bg-slate-900/90 backdrop-blur-md flex items-center justify-center p-4 overflow-hidden">
            <Card className="max-w-4xl w-full max-h-[90vh] flex flex-col shadow-2xl border-0 animate-scale-in">
                <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50">
                    <div>
                        <h2 className="text-xl font-display font-bold text-slate-800">Termos de Uso e Licenciamento</h2>
                        <p className="text-xs text-slate-500 font-bold uppercase tracking-widest mt-1">Versão {template.version}.0 | Plano {user.plan}</p>
                    </div>
                ...
                >
                    <div className="prose prose-slate max-w-none rich-text-content" dangerouslySetInnerHTML={{ __html: template.content }} />
                    ...
                    <div className="mt-12 p-8 border-2 border-dashed border-slate-200 rounded-2xl text-center bg-slate-50">
                        ...
                        {!hasReadToBottom && (
                            <p className="text-brand-blue font-bold text-sm animate-pulse">Role até o final para habilitar a assinatura.</p>
                        )}
                    </div>
                </div>
                ...
                <div className="p-6 bg-slate-50 border-t border-slate-200">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-end">
                        <div className="space-y-3">
                            ...
                            <p className="text-xs text-slate-500 mb-2">Digite seu nome completo para assinar eletronicamente:</p>
                            <Input 
                                value={typedName} 
