    ...
    const handleSubmit = async () => {
        ...
        randomizedQuestions.forEach(q => {
            ...
            if (q.type === QuestionType.MULTIPLE_CHOICE || q.type === QuestionType.TRUE_FALSE) {
                ...
                if (correctOpt && (correctOpt.id === answer || correctOpt.text === answer)) { // Support both ID and Text matching logic just in case
                     // Check if answer is the ID of the option
                     if (answer === correctOpt.id) score++;
                }
            } else if (q.type === QuestionType.NUMERIC) {
                totalAutoGraded++;
                // Simple numeric matching
                const correctVal = q.options?.[0]?.text; // Gabarito stored in first option text
                if (parseFloat(answer) === parseFloat(correctVal || '0')) score++;
            }
            // Subjective (Short Answer) are not auto-graded in score immediately, but we store the text.
        });
        ...
        return (
            <div className="min-h-screen bg-slate-100 flex items-center justify-center p-4">
                <Card className="max-w-lg w-full">
                    ...
                    <div className="bg-blue-50 p-4 rounded-lg mb-6 text-sm text-blue-900 space-y-2">
                    ...
                    <div className="space-y-4">
                        <Input label="Nome Completo" value={studentName} onChange={e => setStudentName(e.target.value)} placeholder="Seu nome" />
                        ...
                        <Button onClick={handleStart} className="w-full justify-center py-3 text-lg">Iniciar Prova</Button>
                    </div>
                </Card>
            </div>
        );
                    ...
                    {exam?.publicConfig?.showFeedback && currentAttempt && (
                        <div className="bg-slate-50 p-4 rounded-lg inline-block">
                            ...
                            <p className="text-3xl font-display font-bold text-brand-blue">
                                {currentAttempt.score} <span className="text-lg text-slate-400">/ {exam.questions.length}</span>
                            </p>
                        </div>
                    )}
                </Card>
    ...
    return (
        <div className="min-h-screen bg-slate-50 flex flex-col">
            ...
            <div className="bg-white border-b border-slate-200 sticky top-0 z-20 px-4 py-3 shadow-sm flex justify-between items-center">
                ...
                {timeLeft !== null && (
                    <div className={`font-mono font-bold text-xl ${timeLeft < 60 ? 'text-red-600 animate-pulse' : 'text-slate-700'}`}>
                        {formatTime(timeLeft)}
                    </div>
                )}
            </div>
            ...
            <div className="max-w-3xl w-full mx-auto p-4 md:p-8 flex-1 space-y-8">
                {randomizedQuestions.map((q, idx) => (
                    <div key={q.id} className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
                        <div className="flex gap-3 mb-4">
                            <span className="bg-slate-100 text-slate-600 font-bold w-8 h-8 rounded-full flex items-center justify-center shrink-0">{idx + 1}</span>
                            <div className="prose prose-slate max-w-none text-slate-800" dangerouslySetInnerHTML={{__html: q.enunciado}} />
                        </div>
                        ...
                        <div className="ml-11">
                            {q.type === QuestionType.MULTIPLE_CHOICE && (
                                <div className="space-y-3">
                                    {q.options?.map((opt, i) => (
                                        <label key={opt.id} className={`flex items-center gap-3 p-3 rounded-lg border cursor-pointer transition-all ${answers[q.id] === opt.id ? 'border-brand-blue bg-blue-50 ring-1 ring-brand-blue' : 'border-slate-200 hover:bg-slate-50'}`}>
                                            <div className={`w-5 h-5 rounded-full border flex items-center justify-center ${answers[q.id] === opt.id ? 'border-brand-blue bg-brand-blue text-white' : 'border-slate-300'}`}>
                ...
                <div className="pt-8 pb-20">
                    <Button onClick={handleSubmit} className="w-full justify-center py-4 text-lg shadow-xl shadow-blue-200">
