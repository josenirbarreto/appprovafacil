            ...
            <Card className="space-y-6">
                <div className="flex items-center gap-6">
                    <div className="relative group shrink-0">
                        {photoUrl ? (
                            ...
                            <div className="w-24 h-24 bg-brand-orange text-white rounded-full flex items-center justify-center text-3xl font-bold uppercase">
                                {name.charAt(0)}
                            </div>
                        )}
                ...
                <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <Input 
                            label="Plano Atual" 
                            value={isAdmin ? 'Acesso Administrativo' : (user?.plan || 'Free')} 
                            disabled 
                            className={isAdmin ? "font-bold text-purple-600 bg-purple-50 border-purple-200" : ""}
                        />
                        <Input 
                            label="Vencimento" 
                            value={isAdmin ? 'Vitalício' : (user?.subscriptionEnd ? new Date(user.subscriptionEnd).toLocaleDateString() : '-')} 
                            disabled 
                            className={isAdmin ? "font-bold text-purple-600 bg-purple-50 border-purple-200" : ""}
                        />
                    </div>
                ...
                <div className="flex justify-between items-center pt-4 border-t border-slate-100">
                    <span className={`text-sm font-medium ${msg.includes('sucesso') ? 'text-green-600' : 'text-red-600'}`}>
                        {msg}
                    </span>
